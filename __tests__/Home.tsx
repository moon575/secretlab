...
test('Test Add To Card', async () => {
  const { asFragment } = render(<Home />);
  await screen.findAllByRole('product-card');
  const documentFragment = asFragment();
  const productCards = Array.from(documentFragment.querySelectorAll('.product-card'));
  const targetProductCard = productCards.find(node => node.querySelector('.product-title')?.textContent === 'iPhone 9');
  const addToCardButton = targetProductCard?.querySelector('button');
  if (addToCardButton) {
    await userEvent.click(addToCardButton);
    await userEvent.click(addToCardButton);
  }
  setTimeout(() => {
    const cartIcon = documentFragment.querySelector('.cart-icon')
    expect(cartIcon).toHaveTextContent('2')
  }, 200);
});
...
(global as any).fetch = jest.fn((url) => {
  switch (url) {
    case 'https://dummyjson.com/products?limit=100&skip=0':
